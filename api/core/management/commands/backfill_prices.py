...
class Command(BaseTaskCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--symbols', type=str, default='', help='Comma-separated symbols (default: all panel symbols)')
        parser.add_argument('--years', type=int, default=10, help='How many years of history to fetch (default: 10)')
        parser.add_argument(
            '--full', action='store_true',
            help='Re-pull the whole window instead of only the missing tail '
                 '(incremental is the default)',
        )
        parser.add_argument('--dry-run', action='store_true', help='Fetch and report counts without writing to the database')
        parser.add_argument(
    ...
    def handle(self, *args, **kwargs):
        ...
        if kwargs['background']:
            ...
            enqueue_bulk(backfill_prices_task, symbols=symbols, years=kwargs['years'], full=kwargs['full'])
            self.stdout.write(self.style.SUCCESS('Enqueued backfill_prices_task'))
            return

        results = backfill_all(symbols=symbols, years=kwargs['years'], dry_run=kwargs['dry_run'], full=kwargs['full'])
