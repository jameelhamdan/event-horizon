...
def test_fetch_day_end_to_end_with_mocked_http():
    if not _DJANGO_READY:
        return
    from services.data import historical, wayback
    from services.data.wayback import WaybackHistoricalService
    ...
    # 'bbc-world' temporarily blocked must not make it flake. The blocked-check
    # lives in BaseHistoricalService.fetch_day (historical module namespace).
    with patch.object(wayback, 'cdx_snapshots', return_value=['20210915113000']), \
         ...
         patch.object(historical, '_is_source_blocked', return_value=False):
