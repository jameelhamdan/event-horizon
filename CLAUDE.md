...
api/                       PYTHONPATH root inside Docker (/app)
  ...
  services/                Stateless Python services (no models)
    ...
    processing/            annotator.py (annotate stage: LLM-free NLP — zero-shot
                           NLI category (primary) + prototype-cosine sub-category
                           + NER + rules + VADER/FinBERT/translation),
...
- `heavy` — NLP/LLM work (scoring, processing, clustering, topic matching, newsletters) — 1 worker (`celery -A app worker -Q heavy --concurrency=1`); ML models load lazily per job, no preloading. This is the only queue any transformer/embedding model (MiniLM, FinBERT, NER, MarianMT, zero-shot NLI — `deberta-v3-base-zeroshot-v2.0` primary + `bge-m3-zeroshot-v2.0` in the refine ensemble) is allowed to load — its container memory cap is sized for that; `bulk`'s is not. The high-volume annotate pass loads only the single primary zeroshot model; the second ensemble member loads lazily when refine actually runs.
- `bulk` — long one-shot jobs and pure dispatchers, 1 worker. Two different shapes share this queue: (1) jobs with no NLP-model footprint that run to completion here (price backfills, LightGBM training/inference) — genuinely long-running but never touch the model stack above, so they're safe at `bulk`'s tighter memory cap; (2) pure dispatchers that select records and fan work out to `heavy` rather than processing inline — the historical-article backfill dispatcher (per-day-chunk fetch/save/annotate runs on `heavy`, bounded to ~10min per chunk), the historical-aggregate dispatcher (per-window clustering/tagging runs on `heavy`), the deferred-annotation dispatcher (per-batch annotation runs on `heavy`), `healing_task` (walks the full historical corpus week by week, re-dispatching still-unannotated/unrefined articles and re-aggregating, all on `heavy` — a manual repair tool, no cron entry, triggered from the dashboard's Actions tab), and `reprocess_articles_task` (same week-by-week walk but re-annotates **every** article including already-terminal `annotated`/`refined` rows — the way to roll a classifier/taxonomy upgrade across the existing corpus without a fresh backfill; optional `rehydrate=True` re-fetches bodies first; dashboard Actions → "Re-annotate corpus", or `manage.py run_task reprocess_articles_task --sync`). Any new `bulk` task that would load one of those NLP models belongs in category (2), not (1).
...
pipeline_tick_task (every 10m) — dispatches due stages from services/stages.py:
  ...
  annotate  (30m, heavy)    — chunks of 8; 100% on-prem NLP (annotator.py):
                              zero-shot NLI category classification (primary, one
                              fast model) + prototype-cosine sub-category within it
...
| ---- | ---------------------------- | ---------------------------------- | ------ |
| Article `category`/`sub_category`/geo/`intensity` | **Category**: zero-shot NLI entailment (primary — the single fast `deberta-v3-base-zeroshot-v2.0`; "what the story is *about*", not prototype word-overlap) with post-hoc evidence gates, in `refiner.classify_zeroshot(single=True)`. **Sub-category**: prototype-cosine (`best_sub`, same MiniLM as clustering) *within* the chosen category. **Geo**: pretrained NER (`wikineural`) → gazetteer geocode + demonym/country fallback. **Intensity**: rule-based. Refine stage re-judges low-margin rows with the full zeroshot ensemble. | `analyzer.py` via `LLM_ROUTES['analyzer_lite']` — one batched prompt per chunk | `services/processing/annotator.py` + `refiner.py` (`classify_zeroshot`/labels/gates in `refiner.py`; sub-category prototypes + intensity priors in `taxonomy.py`; gazetteer/aliases/demonyms in `geocode.py`) / `analyzer.py` |
